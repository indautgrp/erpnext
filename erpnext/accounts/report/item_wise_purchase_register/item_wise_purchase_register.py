...
def execute(filters=None):
	...
	for d in item_list:
		...
		elif d.purchase_order:

			tmp_purchase_receipt=frappe.db.sql("""select parent from `tabPurchase Receipt Item`
			                        where prevdoc_detail_docname = %(po_detail)s
			                        and prevdoc_docname = %(purchase_order)s""", {
...
def get_columns():
	return [_("Item Code") + ":Link/Item:120", _("Item Name") + "::120", _("Item Group") + ":Link/Item Group:100",
		_("Invoice") + ":Link/Purchase Invoice:120", _("Posting Date") + ":Date:80", _("Supplier") + ":Link/Supplier:120", _("Supplier Name") + "::120",
